...
* **Deep Hardening & FinOps:** S3 Buckets are strictly enforced with `aws:SecureTransport` (HTTPS only) and automated Lifecycle Rules (transitioning stale logs to Glacier Deep Archive for cost optimization). Infrastructure is continuously audited via **IAM Access Analyzer**. Defense-in-depth is tested and proven through ephemeral "Deploy & Destroy" architectures utilizing **Application Load Balancers (ALB)** and **AWS WAFv2** (intercepting SQLi payloads at the edge).
